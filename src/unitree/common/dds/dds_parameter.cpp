...
void DdsParameter::Init(const JsonMap& param)
{
    ...
    if (GetMap(param, UT_DDS_PARAM_KEY_PARTICIPANT, participantMap))
    {
        int32_t domainId = 0;
        GetNumberValue<int32_t>(participantMap, UT_DDS_PARAM_KEY_DOMAINID, domainId);
        mDomainId = (uint32_t)domainId;
        mParticipant.SetDomainId(domainId);

        std::string config;
        GetValue<std::string>(participantMap, UT_DDS_PARAM_KEY_CONFIG, config);
        mConfig = config;
        mParticipant.SetConfig(config);

        JsonMap qosMap;
        if (GetMap(participantMap, UT_DDS_PARAM_KEY_QOS, qosMap))
        {
    ...
    JsonMap topicMap;
    if (GetMap(param, UT_DDS_PARAM_KEY_TOPIC, topicMap))
    {
        for (const auto& kv : topicMap)
        {
            ...
            std::string name;
            if (GetValue<std::string>(oneTopicMap, UT_DDS_PARAM_KEY_NAME, name))
            {
                topicParam.SetName(name);
            }

            JsonMap qosMap;
            if (GetMap(oneTopicMap, UT_DDS_PARAM_KEY_QOS, qosMap))
            {
                DdsQosParameter qos;
                qos.Init(qosMap);
    ...
    if (GetArray(param, UT_DDS_PARAM_KEY_PUBLISHER, publisherArray))
    {
        for (const auto& pubAny : publisherArray)
        {
            ...
            JsonMap qosMap;
            if (GetMap(onePubMap, UT_DDS_PARAM_KEY_QOS, qosMap))
            {
                DdsQosParameter qos;
                qos.Init(qosMap);
            ...
            if (GetArray(onePubMap, UT_DDS_PARAM_KEY_WRITER, writerArray))
            {
                for (const auto& writerAny : writerArray)
                {
                    ...
                    GetValue<std::string>(writerMap, UT_DDS_PARAM_KEY_TOPICNAME, topicName);

                    DdsWriterParameter writerParam(topicName);
                    JsonMap writerQosMap;
                    if (GetMap(writerMap, UT_DDS_PARAM_KEY_QOS, writerQosMap))
                    {
    ...
    if (GetArray(param, UT_DDS_PARAM_KEY_SUBSCRIBER, subscriberArray))
    {
        for (const auto& subAny : subscriberArray)
        {
            ...
            JsonMap qosMap;
            if (GetMap(oneSubMap, UT_DDS_PARAM_KEY_QOS, qosMap))
            {
                DdsQosParameter qos;
                qos.Init(qosMap);
            ...
            if (GetArray(oneSubMap, UT_DDS_PARAM_KEY_READER, readerArray))
            {
                for (const auto& readerAny : readerArray)
                {
                    ...
                    std::string topicName;
                    GetValue<std::string>(readerMap, UT_DDS_PARAM_KEY_TOPICNAME, topicName);

                    DdsReaderParameter readerParam(topicName);
                    JsonMap readerQosMap;
                    if (GetMap(readerMap, UT_DDS_PARAM_KEY_QOS, readerQosMap))
                    {
