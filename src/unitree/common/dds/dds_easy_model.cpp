...
{
    std::string fileName = ddsParameterFileName.empty() ? UT_DDS_PARAMETER_CONFIG_FILENAME : ddsParameterFileName;
    UT_THROW(CommonException, std::string("Init from file is not supported in source-only mode. file:") + fileName);
}
