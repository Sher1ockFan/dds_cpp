#ifndef __PND_JSONIZE_HPP__
#define __PND_JSONIZE_HPP__

#define JN_FROM_WEAK(m, name, value) \
    if (m.find(name) != m.end()){ JN_FROM(m, name, value); }

#define JN_FROM(m, name, value) \
    pndbotics::common::FromJson(m[name], value)

#define JN_TO(m, name, value) \
    pndbotics::common::ToJson(value, m[name])
