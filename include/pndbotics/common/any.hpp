#ifndef __PND_ANY_HPP__
#define __PND_ANY_HPP__
 ...
 * static const Any
 */
static const Any PND_EMPTY_ANY = Any();
...
const ValueType& AnyCast(const Any* operand)
{
    ...
    PND_THROW(BadCastException, std::string("AnyCast error. target type is ")
...
ValueType AnyNumberCast(const Any* operand)
{
    ...
    if (IsNumberType(t1) && IsNumberType(t2))
    {
        ...
        {
            PND_THROW(BadCastException, std::string("AnyNumberCast error. unknown number type:", t2.name()));
        }
    }

    PND_THROW(BadCastException, std::string("AnyNumberCast error. not number type"));
}
