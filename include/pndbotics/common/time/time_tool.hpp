#ifndef __PND_TIME_TOOL_HPP__
#define __PND_TIME_TOOL_HPP__
...
namespace common
{
#define PND_NUMER_NANO   1000000000
#define PND_NUMER_MICRO  1000000
#define PND_NUMER_MILLI  1000

//default time format: "YEAR-MON-DAY HOUR:MIN:SEC"
#define PND_TIME_FORMAT_STR  "%d-%02d-%02d %02d:%02d:%02d"

//default precise time format: "YEAR-MON-DAY HOUR:MIN:SEC.[MILLI/MICRO]"
#define PND_TIME_MICROSEC_FORMAT_STR "%d-%02d-%02d %02d:%02d:%02d.%06d"
#define PND_TIME_MILLISEC_FORMAT_STR "%d-%02d-%02d %02d:%02d:%02d.%03d"
...
std::string TimeFormatString(struct tm* tmptr, const char* format = PND_TIME_FORMAT_STR);
std::string TimeFormatString(struct tm* tmptr, uint64_t precise, const char* format);
std::string TimeFormatString(uint64_t sec, const char* format = PND_TIME_FORMAT_STR);

std::string TimeMicrosecondFormatString(const uint64_t& microsec,
    const char* format = PND_TIME_MICROSEC_FORMAT_STR);

std::string TimeMillisecondFormatString(const uint64_t& millisec,
    const char* format = PND_TIME_MILLISEC_FORMAT_STR);
