#ifndef __PND_BLOCK_QUEUE_HPP__
#define __PND_BLOCK_QUEUE_HPP__
...
{
public:
    BlockQueue(uint64_t maxSize = PND_QUEUE_MAX_LEN) :
        mMaxSize(maxSize), mCurSize(0)
    {
        if (mMaxSize == 0)
        {
            mMaxSize = PND_QUEUE_MAX_LEN;
        }
    }
    ...
    T Get(uint64_t microsec = 0)
    {
        ...
        PND_THROW(TimeoutException, "block queue get timeout or interrupted");
    }
