#ifndef __PND_FILE_SYSTEM_HPP__
#define __PND_FILE_SYSTEM_HPP__

#include <pndbotics/common/decl.hpp>

#define PND_FILE_READ_SIZE           65536 //64K
#define PND_FILE_READ_BIGGER_SIZE    262144 //256K

#define PND_FD_INVALID   -1
#define PND_FD_STDIN     STDIN_FILENO
#define PND_FD_STDOUT    STDOUT_FILENO
#define PND_FD_STDERR    STDERR_FILENO

#define PND_PATH_DELIM_CHAR  '/'
#define PND_PATH_DELIM_STR   "/"
#define PND_PATH_TRIM_DELIM_STR   " \t\r\n"
...
enum
{
    PND_OPEN_FLAG_R         = O_RDONLY,
    PND_OPEN_FLAG_W         = O_WRONLY,
    PND_OPEN_FLAG_RW        = O_RDWR,
    PND_OPEN_FLAG_T         = O_TRUNC,
    PND_OPEN_FLAG_A         = O_APPEND,
    PND_OPEN_FLAG_S         = O_SYNC,
    PND_OPEN_FLAG_C         = O_CREAT,
    PND_OPEN_FLAG_CW        = O_CREAT | O_WRONLY,
    PND_OPEN_FLAG_CWT       = O_CREAT | O_WRONLY | O_TRUNC,
    PND_OPEN_FLAG_CA        = O_CREAT | O_RDWR | O_APPEND,
    PND_OPEN_FLAG_CS        = O_CREAT | O_RDWR | O_SYNC,
    PND_OPEN_FLAG_CAS       = O_CREAT | O_RDWR | O_APPEND | O_SYNC,
    PND_OPEN_FLAG_CTS       = O_CREAT | O_RDWR | O_TRUNC | O_SYNC
};

enum
{
    PND_OPEN_MODE_NONE = 0,
    PND_OPEN_MODE_RW   = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH,
    PND_OPEN_MODE_RWX  = S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH
};

enum
{
    PND_OPEN_MASK_000 = 0,
    PND_OPEN_MASK_022 = 0022
};
...
public:
    ...
    void Makedir(const std::string& dirName, uint32_t mode = PND_OPEN_MODE_RWX,
        bool ignoreExist = true);
    void MakedirRecurse(const std::string& dirName, uint32_t mode = PND_OPEN_MODE_RWX);
