#ifndef __PND_FILE_HPP__
#define __PND_FILE_HPP__

#include <pndbotics/common/filesystem/filesystem.hpp>
...
public:
    ...
    void Open();
    void Open(int32_t flag, uint32_t mode = PND_OPEN_MODE_NONE);
    void Open(const std::string& fileName, int32_t flag = PND_OPEN_FLAG_R,
        uint32_t mode = PND_OPEN_MODE_NONE);
...
void SaveFile(const std::string& fileName, const char* s, int64_t len,
    uint32_t mode = PND_OPEN_MODE_RW);
