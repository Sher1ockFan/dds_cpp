#ifndef __PND_DDS_ENTITY_HPP__
#define __PND_DDS_ENTITY_HPP__
...
#include <pndbotics/common/dds/dds_exception.hpp>
#include <pndbotics/common/dds/dds_callback.hpp>
#include <pndbotics/common/dds/dds_qos.hpp>
#include <pndbotics/common/dds/dds_traits.hpp>

#define __PND_DDS_NULL__ ::dds::core::null

/*
 * dds wait sub/pub matched default time slice.
 * default 10000 us
 */
#define __PND_DDS_WAIT_MATCHED_TIME_SLICE 10000
#define __PND_DDS_WAIT_MATCHED_TIME_MAX   1000000
...
public:
    ...
    explicit DdsTopic(const DdsParticipantPtr& participant, const std::string& name, const DdsTopicQos& qos) :
        mNative(__PND_DDS_NULL__)
    {
        PND_DDS_EXCEPTION_TRY
        ...
        PND_DDS_EXCEPTION_CATCH(mLogger, true)
    }
...
public:
    ...
    bool Write(const MSG& message, int64_t waitMicrosec)
    {
        ...
        PND_DDS_EXCEPTION_CATCH(mLogger, false)
...
private:
    ...
    {
        if (waitMicrosec < __PND_DDS_WAIT_MATCHED_TIME_SLICE)
        {
            return;
        }

        int64_t waitTime = (waitMicrosec / 2);
        if (waitTime > __PND_DDS_WAIT_MATCHED_TIME_MAX)
        {
            waitTime = __PND_DDS_WAIT_MATCHED_TIME_MAX;
        }

        while (waitTime > 0 && mNative.publication_matched_status().current_count() == 0)
        {
            MicroSleep(__PND_DDS_WAIT_MATCHED_TIME_SLICE);
            waitTime -=__PND_DDS_WAIT_MATCHED_TIME_SLICE;
        }
    }
...
public:
    ...
    void SetQueue(int32_t len)
    {
        ...
        mDataQueueThreadPtr = CreateThreadEx("rlsnr", PND_CPU_ID_NONE, queueThreadFunc);
    }
