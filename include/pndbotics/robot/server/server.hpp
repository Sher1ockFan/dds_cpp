#ifndef __PND_ROBOT_SDK_SERVER_HPP__
#define __PND_ROBOT_SDK_SERVER_HPP__

#include <pndbotics/robot/server/server_base.hpp>
#include <pndbotics/robot/server/lease_server.hpp>

#define PND_ROBOT_SERVER_REG_API_HANDLER_NO_LEASE(apiId, handler)            \
    PND_ROBOT_SERVER_REG_API_HANDLER(apiId, handler, false)

#define PND_ROBOT_SERVER_REG_API_BINARY_HANDLER_NO_LEASE(apiId, handler)     \
    PND_ROBOT_SERVER_REG_API_BINARY_HANDLER(apiId, handler, false)

#define PND_ROBOT_SERVER_REG_API_HANDLER(apiId, handler, checkLease)         \
    RegistHandler(apiId, std::bind(handler, this, std::placeholders::_1, std::placeholders::_2), checkLease)

#define PND_ROBOT_SERVER_REG_API_BINARY_HANDLER(apiId, handler, checkLease)  \
...
#endif//__PND_ROBOT_SDK_SERVER_HPP__
